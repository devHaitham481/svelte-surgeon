...
const CONFIG_DIRECTORY = join(homedir(), ".svelte-doctor");
const CONFIG_FILE = join(CONFIG_DIRECTORY, "config.json");
const SKILL_REPO = "millionco/svelte-doctor";
...
export const maybePromptSkillInstall = async (shouldSkipPrompts: boolean): Promise<void> => {
  ...
  logger.dim(
    `   Install the ${highlighter.info("svelte-doctor")} skill to teach Cursor, Claude Code, Copilot,`,
  );
  logger.dim("   Ami, and other AI agents how to diagnose and fix these Svelte issues.");
  logger.break();
